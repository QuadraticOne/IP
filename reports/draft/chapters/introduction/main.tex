...
Since both $S$ and $C$ are hypercubes, it is trivial sample them uniformly.
...
This knowledge can be used to create a set of viable solutions $V_c$, specific to the constraint, such that any solution vector which is contained within $V_c$ satisfies the constraint by definition.
\begin{equation}
    V_c=\{s\;|\;s\in S,\;h(c,s)=\text{satisfied}\}
\end{equation}
While envisaging such a set is trivial, sampling from it is not.
One could learn a generator function $g(c)$ which maps from an arbitrary latent space $L$ (from which samples can be easily drawn) to somewhere in $V_c$:
\begin{equation}
